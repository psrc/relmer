...
vignette: >
  %\VignetteIndexEntry{create-a-fancy-map-with-elmergeo-data}
...
In the vignette [Create a Map with ElmerGeo Data](https://psrc.github.io/psrcelmer/articles/create-a-map-with-elmergeo-data.html), we showed you how to make a simple leaflet map with ElmerGeo data. What if you want to make a nicer looking map?

In this example we are going to use some of our homegrown libraries psrccensus and psrcplot to make a map of Census data using PSRC colors. This map will show usually working at home commute shares by Census Tract from the 5-year ACS 2017-2021 dataset. (It doesn't include workers who occasionally work at home, this messes up a lot).
...
This first step gets commute mode totals by tract for each mode. 
...
home_workers_join<-merge(all_workers_tract, home_workers_tract, by= 'GEOID', suffixes=c('_all', '_home'))
```
